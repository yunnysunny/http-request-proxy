...
const _doProxy = function(options, agentUrl, req, res) {
    ...
    const {
        dataPrepare, timeout, onError, headerPrepare
    } = (options);
    const reqOptions = {
        url: agentUrl,
        method,
 ...
 * @throws {Error} When the value in option.urlsToProxy is not a string or not a function with return value of Promise, an error will raise up.
 */
    ...
    return function(req, res, next) {
        ...
        checkUrl(req, urlsToProxy, urlKeys, function(err, backUrl) {
            if (err) {
                ...
                return res.status(503).end('proxy ' + path +' failed: ' + err);
            }
            if (!backUrl) {
                return next();
            }
